            ...
            <main className={estilos.main}>
                <div className={estilos.Grid}>
                    ...
                    {nombres.slice(0, 101).map((element) => (
                        <div className="flex">
                            <button id={`${element.types[0].type.name}`} key={element.order}
                                onClick = {()=> {
                            ...
                            <div id={element.order} className="hidden">
                                <div className="flex flex-col w-4/5 self-center" id={`${element.types[0].type.name}`} key={element.order}>
                                    <img src={element.sprites.other.dream_world.front_default} alt="sprite" srcset="" />
                                    <p>Name: {element.name}</p>
                                    <p>Base Experience: {element.base_experience}</p>
                                    <p>Weight: {element.weight}</p>
                                    <p>height: {element.height}</p>
                                    <div>
                                        <p>Ability: {element.abilities[0].ability.name}</p>
                                        <p>Hidden Ability: {element.abilities[1] ? element.abilities[1].ability.name : "none"}</p>
                                    </div>
                                    <div>
                                        <p id={element.types[0].type.name} >{element.types[0].type.name}</p>
                                        <p id={element.types[1] ? element.types[1].type.name : ""} >{element.types[1] ? element.types[1].type.name : ""}</p>
                                    </div>
                                    <div>
                                        <p>{element.stats[0].stat.name}: {element.stats[0].base_stat}</p>
                                        <p>{element.stats[1].stat.name}: {element.stats[1].base_stat}</p>
                                        <p>{element.stats[2].stat.name}: {element.stats[2].base_stat}</p>
                                        <p>{element.stats[3].stat.name}: {element.stats[3].base_stat}</p>
                                        <p>{element.stats[4].stat.name}: {element.stats[4].base_stat}</p>
                                        <p>{element.stats[5].stat.name}: {element.stats[5].base_stat}</p>
                                    </div>
                                </div>
                            </div>
